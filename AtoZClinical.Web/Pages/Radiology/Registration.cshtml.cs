...
{
    private readonly RadiologyTestService _service;

    public RegistrationModel(ClinicContextService clinicContext, RadiologyTestService service) : base(clinicContext)
    {
        _service = service;
    }

    [BindProperty]
    public RadiologyTestInput Input { get; set; } = new();

    public List<RadiologyTest> Records { get; private set; } = [];
    ...
    private async Task LoadRecord(Guid clinicId, Guid id)
    {
        ...
        RecordId = item.Id;
        Input = RadiologyTestInput.FromEntity(item);
    }
    ...
    private async Task PrepareNew(Guid clinicId)
    {
        ...
        Input = new RadiologyTestInput { TestNo = next, Category = ClinicLookup.RadiologyCategories[0] };
    }
    ...
    private async Task<IActionResult> SaveCoreAsync()
    {
        ...
        {
            ModelState.AddModelError(string.Empty, "Radiology Code and Radiology Name are required.");
            await LoadAsync(clinicId.Value);
            SetFormViewData("Radiology Registration", null, null, Input?.UpdatedAt);
            return Page();
        }
    ...
    public sealed class RadiologyTestInput
    {
        ...
        public static RadiologyTestInput FromEntity(RadiologyTest t) => new()
        {
            TestNo = t.TestNo,
            TestCode = t.TestCode,
            TestName = t.TestName,
        ...
        public RadiologyTest ToEntity(Guid? id) => new()
        {
            Id = id ?? Guid.Empty,
            TestNo = TestNo,
            TestCode = (TestCode ?? string.Empty).Trim(),
            TestName = (TestName ?? string.Empty).Trim(),
            Category = Category ?? ClinicLookup.RadiologyCategories[0],
            Fee = Fee,
            Note = Note
        };
    }
