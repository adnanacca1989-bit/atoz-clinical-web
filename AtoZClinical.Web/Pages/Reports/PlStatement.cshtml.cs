...
public class PlStatementModel : PageModel
{
    ...
    private async Task<IActionResult> RunAsync()
    {
        ...
        var pharmacyBills = await _db.PharmacyBills
            .Where(b => b.ClinicId == clinicId && b.BillDate >= FromDate.Date && b.BillDate <= ToDate.Date)
            .ToListAsync();

        if (!string.IsNullOrWhiteSpace(DoctorName))
            pharmacyBills = pharmacyBills.Where(b => b.DoctorName?.Contains(DoctorName, StringComparison.OrdinalIgnoreCase) == true).ToList();
        if (!string.IsNullOrWhiteSpace(PatientName))
            pharmacyBills = pharmacyBills.Where(b => b.PatientName?.Contains(PatientName, StringComparison.OrdinalIgnoreCase) == true).ToList();

        foreach (var bill in pharmacyBills)
            Results.Add(new PlRow("Revenue", "Pharmacy Sales", $"Bill #{bill.BillNo} — {bill.PatientName}", bill.TotalAmount));
